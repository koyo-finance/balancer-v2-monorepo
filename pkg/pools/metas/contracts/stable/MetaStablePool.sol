...
import { PoolPriceOracle } from "@koyofinance/exchange-vault-pool-utils/contracts/oracle/PoolPriceOracle.sol";

import { StablePool } from "@koyofinance/exchange-vault-pool-stable/contracts/stable/StablePool.sol";
import { MetaStablePoolOracleMiscData } from "./MetaStablePoolOracleMiscData.sol";
import { StableOracleMath } from "../math/StableOracleMath.sol";
...
contract MetaStablePool is StablePool, PoolPriceOracle {
    ...
    function _updateOracle(
        ...
        if (miscData.oracleEnabled() && block.number > lastChangeBlock) {
            (int256 logSpotPrice, int256 logBptPrice) = StableOracleMath.calcLogPrices(
                currentAmp,
                balance0,
                balance1,
