import { PoolSpecialization } from '@koyofinance/vault-js';
import { sharedBeforeEach } from '@koyofinance/exchange-vault-common/sharedBeforeEach';
import { calculateInvariant } from '@koyofinance/exchange-vault-helpers/src/models/pools/stable/math';
import StablePool, { SWAP_INTERFACE } from '@koyofinance/exchange-vault-helpers/src/models/pools/stable/StablePool';
import { RawStablePoolDeployment } from '@koyofinance/exchange-vault-helpers/src/models/pools/stable/types';
import TokenList from '@koyofinance/exchange-vault-helpers/src/models/tokens/TokenList';
import { BigNumberish, bn, fp, pct } from '@koyofinance/exchange-vault-helpers/src/numbers';
...
} from '@koyofinance/exchange-vault-helpers/src/time';
        ...
        context('when requesting a reasonable change duration', () => {
          ...
          context('when requesting a valid amp', () => {
            ...
            context('when decreasing the amp', () => {
              context('when decreasing the amp by 4x', () => {
                const newAmp = AMPLIFICATION_PARAMETER.div(4);
          ...
          context('when requesting an invalid amp', () => {
            ...
            describe('rate limits', () => {
              ...
              it('reverts when increasing the amp by more than 4x in a single day', async () => {
                const newAmp = AMPLIFICATION_PARAMETER.mul(4).add(1);
                const endTime = startTime.add(DAY);

                await expect(pool.startAmpChange(newAmp, endTime)).to.be.revertedWith('AMP_RATE_TOO_HIGH');
              });

              it('reverts when increasing the amp by more than 4x daily over multiple days', async () => {
                const newAmp = AMPLIFICATION_PARAMETER.mul(9).add(1);
                const endTime = startTime.add(DAY * 2);

                await expect(pool.startAmpChange(newAmp, endTime)).to.be.revertedWith('AMP_RATE_TOO_HIGH');
              });

              it('reverts when decreasing the amp by more than 4x in a single day', async () => {
                const newAmp = AMPLIFICATION_PARAMETER.div(4).sub(1);
                const endTime = startTime.add(DAY);

                await expect(pool.startAmpChange(newAmp, endTime)).to.be.revertedWith('AMP_RATE_TOO_HIGH');
              });

              it('reverts when decreasing the amp by more than 4x daily over multiple days', async () => {
                const newAmp = AMPLIFICATION_PARAMETER.div(9).sub(1);
                const endTime = startTime.add(DAY * 2);
