...
library GPv2Transfer {
    ...
    function fastTransferFromAccount(
        ...
        if (transfer.balance == GPv2Order.BALANCE_ERC20) {
            ...
            IVaultCoW.UserBalanceOp[]
                memory balanceOps = new IVaultCoW.UserBalanceOp[](1);

            IVaultCoW.UserBalanceOp memory balanceOp = balanceOps[0];
            balanceOp.kind = transfer.balance == GPv2Order.BALANCE_EXTERNAL
                ? IVaultCoW.UserBalanceOpKind.TRANSFER_EXTERNAL
                : IVaultCoW.UserBalanceOpKind.TRANSFER_INTERNAL;
    ...
    function transferFromAccounts(
        ...
        IVaultCoW.UserBalanceOp[] memory balanceOps = new IVaultCoW.UserBalanceOp[](
            transfers.length
        );
        ...
        for (uint256 i = 0; i < transfers.length; i++) {
            ...
            if (transfer.balance == GPv2Order.BALANCE_ERC20) {
                ...
                IVaultCoW.UserBalanceOp memory balanceOp = balanceOps[
                    balanceOpCount++
                ];
                balanceOp.kind = transfer.balance == GPv2Order.BALANCE_EXTERNAL
                    ? IVaultCoW.UserBalanceOpKind.TRANSFER_EXTERNAL
                    : IVaultCoW.UserBalanceOpKind.WITHDRAW_INTERNAL;
                balanceOp.asset = transfer.token;
                balanceOp.amount = transfer.amount;
                balanceOp.sender = transfer.account;
                balanceOp.recipient = payable(recipient);
    ...
    function transferToAccounts(IVaultCoW vault, Data[] memory transfers)
        internal
    {
        IVaultCoW.UserBalanceOp[] memory balanceOps = new IVaultCoW.UserBalanceOp[](
            transfers.length
        ...
        for (uint256 i = 0; i < transfers.length; i++) {
            ...
            if (address(transfer.token) == BUY_ETH_ADDRESS) {
                ...
                balanceOp.kind = IVaultCoW.UserBalanceOpKind.DEPOSIT_INTERNAL;
                balanceOp.asset = transfer.token;
                balanceOp.amount = transfer.amount;
    ...
    function truncateBalanceOpsArray(
        IVaultCoW.UserBalanceOp[] memory balanceOps,
        uint256 newLength
    ) private pure {
